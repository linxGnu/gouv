...
func TCPInit(loop *UvLoop, flags *uint, data interface{}) (*UvTCP, error) {
	...
	res.Stream, res.Loop, res.TCP, res.Handle = (*C.uv_stream_t)(unsafe.Pointer(t)), loop.GetNativeLoop(), t, Handle{(*C.uv_handle_t)(unsafe.Pointer(t)), t.data, res}
...
func (t *UvTCP) KeepAlive(enable int, delay uint) C.int {
	return C.uv_tcp_keepalive(t.TCP, C.int(enable), C.uint(delay))
}
...
func (t *UvTCP) SimultaneousAccepts(enable int) C.int {
	return C.uv_tcp_simultaneous_accepts(t.TCP, C.int(enable))
}
...
func (t *UvTCP) Bind(sockAddr SockaddrIn, flags uint) C.int {
	return C.uv_tcp_bind(t.TCP, sockAddr.GetSockAddr(), C.uint(flags))
}
...
func (t *UvTCP) Connect(req *UvConnect, sockAddr SockaddrIn, cb func(*Request, int)) C.int {
	...
	return uv_tcp_connect(req.Connect, t.TCP, sockAddr.GetSockAddr())
}
